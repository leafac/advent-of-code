...
while (statesToExplore.length > 0) {
  ...
  const stateToExploreSerialized = JSON.stringify({
    ...
    stepsRemainingInDirection: stateToExplore.stepsRemainingInDirection,
  });
  ...
  for (const nextStateToExplore of [
    {
      ...
      direction: "up",
      stepsRemainingInDirection:
        stateToExplore.direction === "up"
          ? stateToExplore.stepsRemainingInDirection - 1
          : 3,
      accumulatedHeatLoss,
    },
    {
    ...
    if (
      ...
      nextStateToExplore.stepsRemainingInDirection > 0
    )
